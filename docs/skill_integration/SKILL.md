...
description: 專為 Advantest V93K 等高端 ATE 測試卡設計的訊號用元件自動佈排系統開發與部署指南。當使用者需要進行 ATE PCB 的自動化佈排研究、架構設計、演算法開發（啟發式、SA、ML、RL）、Cadence Allegro 整合、PoC 驗證、或生成相關技術文檔與投影片時，請觸發此技能。
---
...
- 若使用者需要簡報，請參考 `templates/slides_structure.md` 的大綱，使用 `slide_initialize` 與 `slide_edit` 工具生成專業投影片。
